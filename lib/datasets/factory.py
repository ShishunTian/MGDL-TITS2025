...
for year in ['2007']:
  for split in ['train_s', 'test_s', 'train_combine_fg', 'train_cg_fg']:
    name = 'cs_{}_{}'.format(year, split)
    __sets[name] = (lambda split=split: cityscape(split, year))

for year in ['2007']:
  # for split in ['train_t', 'test_t', 'train_combine','train_cg']:
  for split in ['train_t', 'test_t','test_s','error','fn_test']:
...
for year in ['2007']:
  for split in ['train_s', 'train_t', 'test_s','test_t']:
    name = 'cs_rtts_{}_{}'.format(year, split)
    __sets[name] = (lambda split=split: rtts(split, year))
