...
def vis_detections(im, class_name, dets, thresh=0.8):
    ...
    label = {
        'bus': (193,182,255),  #浅粉红
        'bicycle':(204,50,153), # 深兰花紫
        'car':(255,0,0),  #春纯蓝
        'motorcycle':(255,255,0), # 青色
        'person':(0,255,255), #纯黄
        'rider':(30,105,210), #巧克力
        'train':(0,165,255),  #橙色
        'truck':(0,255,0) #绿色
    }
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        score = dets[i, -1]
        if score > thresh:
            cv2.rectangle(im, bbox[0:2], bbox[2:4], label[class_name], 2)
            # cv2.putText(im, class_name, (bbox[0], bbox[1]), cv2.FONT_ITALIC,
            #             1, label[class_name], thickness=2)
