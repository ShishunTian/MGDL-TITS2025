...
from model.utils.transformer import MultiHeadAttention
...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic,lc,gc, la_attention = False, mid_attention = False):
        ...
        self.cross_attention = MultiHeadAttention(4096,dropout=0.1,version='v2')
    ...
    def forward_unsup(self, im_data, im_info, gt_boxes, num_boxes,gt_boxes_lh,num_boxes_lh,im_data_w,im_info_w,teacher_model,target=False,eta=1.0):
        ...
        cls_prob = F.softmax(cls_score, 1)
        pooled_feat_lh_temp = torch.tensor([0.])
        ...
        if self.training:
            ...
            if fg_num_rois_lh > 0:
                ...
                # cross_feat,edge = self.cross_attention(pooled_feat_lh,pooled_feat_lh,pooled_feat_lh)
                # if len(cross_feat.shape) == 1:
                #     cross_feat = cross_feat.unsqueeze(0)
                pooled_feat_tea_temp = pooled_feat_tea
                q = nn.functional.normalize(pooled_feat_lh)
                k = nn.functional.normalize(pooled_feat_tea_temp)
                k = k.detach()
                # positive logits: Nx1
                logits = torch.matmul(q,k.t())
                # l_pos = torch.einsum("nc,nc->n", [q, k.detach()]).unsqueeze(-1)
                # negative logits: NxK
                # l_neg = torch.einsum("nc,ck->nk", [q, k.T.detach()])
        ...
        return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,RCNN_loss_cls_lh,contrast_loss
